...
## Visualize the anatomical distribution of effects
```{r vizmorph2,results='hide'}
betas<-antsImageClone(mask)
betas[mask==1]<-mylmresults$beta.t[1,]
thresh<-paste(mean(mylmresults$beta.t[1,]),'x',
  max(mylmresults$beta.t[1,]),sep='')
plotANTsImage(img,functional=list(),
  outname=paste(figpath,'loadimages.jpg',sep=''))
```
